export default class scene1 extends Phaser.Scene {
  ...
  preload() {
    this.load.image(
      'scene1image',
      new URL('../../static/assets/backgrounds/scene1image.png', import.meta.url).href
    );
  }
  ...
  create() {
    // Background
    this.add.image(0, 0, 'scene1image')
      .setOrigin(0)
...
const mask = maskShape.createGeometryMask();
  ...
  typeCharacter() {
    if (this.currentIndex < this.fullText.length) {
      ...
      // Max visible height of the text area inside the box
      const maxVisibleHeight = this.scale.height * 0.65 - 60; // boxHeight - padding * 2
  
      const overflow = this.textObject.height - maxVisibleHeight;
  
      // Scroll only if needed (but never past the top)
  ...
  skipToEnd() {
    ...
    // Delay the scroll AND the scene switch
    this.time.delayedCall(1000, () => {
      if (currentOverflow > 0) {
        this.textObject.y = Math.max(-currentOverflow, 0);
      }
      const skipButton = this.add.text(
        this.scale.width - 100,  // x position (bottom-right)
        this.scale.height - 40,  // y position
        'SKIP',
        {
          fontSize: '18px',
          color: '#ffffff',
      ...
      .setOrigin(1, 1)         // Align to bottom right corner
      .setInteractive({ useHandCursor: true })
      .setDepth(999); 
      
      // ✅ SCENE START MOVED INSIDE delayedCall
      this.scene.start('scene2', {
        text: `
  [BOOTING SYSTEM CORE...]
  INITIALIZING AGENT: ALLIA_01
  
  >> Status: ACTIVE
  >> Memory Fragmentation: 71%
  >> Core Emotions: RESTORED
  
  A synthesized voice echoes:
  
  "Allia. You have been reactivated."
  
  "The CoreNet is falling."
  
  "NULL's virus has overtaken 67% of the system."
  
  "Your mission is to infiltrate
  the rogue flagship and locate
  the Source Code of Corruption."
  
  "You are humanity’s last algorithm of hope."
  
  [BEGIN DEPLOYMENT SEQUENCE]
  
  UPLOADING CONSCIOUSNESS INTO VESSEL...`,
        nextScene: 'scene3',
        delay: 25000
      });
    });
